...
def init_neptune(cfg):
    neptune.set_project('yoniosin/amygdala')
    neptune.init('yoniosin/amygdala')
    neptune.create_experiment(params=flatten(cfg, reducer='path'),
...
def update_cfg(cfg: EEGLearnerConfig):
    ...
    cfg.learner.logger_path = f'{cfg.learner.runs_dir}/eeg/run#{cfg.learner.run_num} ({cfg.net.embedding_size > 0})'
